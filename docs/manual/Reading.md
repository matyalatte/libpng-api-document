...
handling in the Customizing Libpng section (\ref customizing) for more information
...
\note
the values can be passed to either the fixed or floating
...
`png_set_add_alpha()` is called to insert two filler bytes, either before
